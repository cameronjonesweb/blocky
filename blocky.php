...
class blocky {
	...
	function __construct() {
		...
		define( 'CJW_BACB_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
		define( 'CJW_BACB_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
		define( 'CJW_BACB_PLUGIN_VER', '1.3.0' );
		//Actions
		add_action( 'add_meta_boxes', array( $this, 'blocky_dynamic_add_custom_box' ), 1 );
		add_action( 'save_post', array( $this, 'blocky_dynamic_save_postdata' ), 50 );
		add_action( 'admin_enqueue_scripts', array( $this, 'blocky_admin_resources' ), 99 );
		add_action( 'wp_ajax_nopriv_ajax_wp_editor', array( $this, 'blocky_ajax_wp_editor' ) );
		add_action( 'wp_ajax_ajax_wp_editor', array( $this, 'blocky_ajax_wp_editor' ) );
		add_action( 'admin_notices', array( $this, 'blocky_admin_notice' ) );
		add_action( 'admin_init', array( $this, 'blocky_admin_notice_ignore' ) );
		add_action( 'admin_menu', array( $this, 'blocky_admin_menu' ) );

		//Filters
		if( esc_attr( get_option( 'blocky_disable_content_filter' ) ) != true ) {
			add_filter( 'the_content', array( $this, 'blocky_content_filter' ), 1 );
		}
		add_filter( 'tiny_mce_before_init', array( $this, 'blocky_get_TinyMCE_Settings' ) );
		if( defined('WPSEO_VERSION') && intval('WPSEO_VERSION') < 3 ) {
			add_filter( 'wpseo_pre_analysis_post_content', array( $this, 'blocky_yoast_seo_content_filter' ) );
		}
	...
	function blocky_body_classes( $classes ) {
		$classes[] = 'blocky-' . CJW_BACB_PLUGIN_VER;
	    return $classes;
	}
	...
	function blocky_dynamic_add_custom_box() {
		...
		if( isset( $blocky_post_types ) && !empty( $blocky_post_types ) ) {
			foreach( $blocky_post_types as $blocky_post_type => $active ){
				add_meta_box( 'blocky_meta_box', __( 'Additional Content', 'blocky' ), array( $this, 'blocky_dynamic_inner_custom_box' ), $blocky_post_type, 'normal', 'high' );
			}
		}
	}
	...
	function blocky_admin_menu() {
		...
		add_menu_page( __( 'Blocky! Settings', 'blocky' ), __( 'Blocky!', 'blocky' ), 'administrator', 'blocky-settings', array( $this, 'blocky_settings_page' ) , $icon );

		//call register settings function
		add_action( 'admin_init', array( $this, 'blocky_settings' ) );
	}
	...
	<form method="post" action="options.php">
	    ...
	    <table class="form-table">
	       ...
	       <!-- <tr valign="top">
	        ...
	        <tr valign="top">
	            <th scope="row"><?php _e( 'Disable content filter (if you\'re using <code>get_additional_content()</code>)', 'blocky' );?></th>
	            <td><input type="checkbox" name="blocky_disable_content_filter" value="true" <?php checked( esc_attr( get_option( 'blocky_disable_content_filter' ) ), 'true' ); ?> /></td>
	        </tr>
	    </table>
